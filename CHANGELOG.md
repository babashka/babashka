...
- `babashka.ffi`: a pointer is a `java.lang.foreign.MemorySegment` instead of a long. A pointer from `alloc` knows its length, so a read past the end or after `free` throws instead of corrupting memory, and `read` and `write` on it skip a per-call segment rebuild. A pointer from C has length 0 and `read` sizes it from the type. New: `segment`, `reinterpret`, `slice`, `address`. A number where a pointer is expected is an error; wrap a raw address with `segment`
