...
- TODO: drop the 224 variadic FFM descriptors, about 290 KB, from a build
  that links libffi. They exist for the FFM variadic fallback, which only a
  build without libffi uses. The static musl build links no libffi, so make
  it conditional on BABASHKA_FEATURE_LIBFFI, which script/uberjar already
  knows. Generated by script/gen_ffi_metadata.clj
