...
- Tasks: `:tasks {:imports ([some-other.lib :refer [some-task]])}` imports tasks a library ships as `some_other/lib/tasks.edn` on the classpath. A referred task brings its transitive `:depends` along. The library declares its deps in its own deps.edn
