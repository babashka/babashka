...
- `babashka.ffi`: Pointers are now native `java.lang.foreign.MemorySegment` values with a size. Access outside this size throws. Pointers from C have size zero. Use `reinterpret` before you access these pointers. New functions: `segment`, `reinterpret`, `slice`, `address`, `size`, and `pointer?`. Pointer arguments reject numbers, heap segments, pointers of a closed arena, and pointers of an arena confined to another thread. Use `segment` to convert a raw address.
- `babashka.ffi`: arenas for scoped allocation. `confined-arena`, `shared-arena`, `auto-arena` and `global-arena` pass through to `java.lang.foreign.Arena`, `alloc` takes an arena and a type keyword, and closing an arena in `with-open` releases everything allocated in it
