...
- `babashka.ffi`: `read-bytes` and `write-bytes` copy byte arrays to and from native memory, and `byte-buffer` gives a `java.nio.ByteBuffer` view over it that copies nothing
