...
- `babashka.ffi`: Pointers are now `java.lang.foreign.MemorySegment` values with a size. Access outside this size throws. Pointers from C have size zero. Use `reinterpret` before you access these pointers. New functions: `segment`, `reinterpret`, `slice`, `address`, `size`, and `pointer?`. Pointer arguments reject numbers. Use `segment` to convert a raw address. A heap segment is not a pointer and is refused
