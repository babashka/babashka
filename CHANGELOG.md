...
- `babashka.ffi`: a pointer is a `java.lang.foreign.MemorySegment` with a size instead of a long. A read or write past the size throws instead of corrupting memory. A pointer from C has size 0 and needs `reinterpret` before a read or write. New: `segment`, `reinterpret`, `slice`, `address`, `size`, `pointer?`. A number where a pointer is expected is an error; wrap a raw address with `segment`
