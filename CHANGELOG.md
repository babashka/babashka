...
- TODO: drop the 224 variadic FFM descriptors, about 290 KB, from a build
  that links libffi. They serve the FFM variadic fallback, which only a build
  without libffi uses: a distribution package that sets BABASHKA_LIBFFI=none
  and still loads shared libraries. A static musl binary cannot load one at
  all, so it needs neither path. Gate on BABASHKA_FEATURE_LIBFFI, which
  script/libffi_archive.sh already exports. Generated by
  script/gen_ffi_metadata.clj
