...
- `babashka.ffi`: Adds arenas for scoped allocation. `confined-arena`, `shared-arena`, `auto-arena`, and `global-arena` return `java.lang.foreign.Arena` instances. `alloc` accepts an arena, a type keyword, and an alignment. When `with-open` closes an arena, the arena releases all its memory.
