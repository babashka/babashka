        ...
        (doseq [n namespaces]
          (let [orchestra? (str/starts-with? (str n) "orchestra")]
            ...
            (when-not orchestra?
              ...
              (when (and (= 'clojure.data.json-test n) (windows?))
                (println "Skipping data.json-test/pretty-print-nonescaped-unicode on Windows (line ending mismatch)")
                (alter-meta! (resolve 'clojure.data.json-test/pretty-print-nonescaped-unicode) assoc :skip-bb true))
