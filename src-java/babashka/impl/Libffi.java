...
/** The two libffi entry points that babashka.ffi calls to pass a struct by
 * value, bound at link time. Only functional in a native image, and only in
 * one that links libffi: without the archive the symbols do not resolve.
 * babashka.impl.libffi, the only caller, is on the classpath only for a
 * build with BABASHKA_LIBFFI, which keeps these methods unreachable
 * otherwise. See script/setup-libffi. */
