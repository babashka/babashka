...
;;
;; Usage:
;;   bb script/resync.clj lib <lib> <new-sha> [copies-dir]
;;   bb script/resync.clj dir <copies-dir> <new-sha>
;;
;; lib takes its pin from test-resources/lib_tests/bb-tested-libs.edn and
;; merges the copies under test-resources/lib_tests/<copies-dir>, which
;; defaults to the library's name.
;; dir takes its pin from <copies-dir>/upstream.edn and merges everything
...
(let [[mode target new-sha copies-arg] *command-line-args*]
  (when-not (and mode target new-sha)
    (die "Usage: bb script/resync.clj lib <lib> <new-sha> [copies-dir]\n       bb script/resync.clj dir <copies-dir> <new-sha>"))
  (case mode
    "lib"
    (let [lib (symbol target)
          entry (get (edn/read-string (slurp registry)) lib)
          _ (when-not entry (die "No entry for" target "in" registry))
          {:keys [git-sha git-url test-paths]} entry
          _ (when-not (seq test-paths)
              (die target "has no :test-paths, so where its copies came from upstream is unknown"))
          copies (fs/file "test-resources/lib_tests" (or copies-arg (name lib)))
    ...
    "dir"
    (let [descriptor (fs/file target "upstream.edn")
          _ (when-not (fs/exists? descriptor) (die "No upstream.edn in" target))
          {:keys [lib git-url git-sha upstream-path]} (edn/read-string (slurp (str descriptor)))
          results (merge-copies {:checkout (checkout-at lib git-url git-sha)
                                 :old-sha git-sha :new-sha new-sha
                                 :copies (fs/file target) :rel-root (fs/file target)
