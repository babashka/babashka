...
defaults from its super POM. It looks an expression up in this order, see
...
`parent.version`, `basedir` and `project.basedir`. It let these values win
over a property with the same name. grenadine resolves the same fixed set.
...
Resolution reads dependencies, repositories, parents and relocations. In
those elements of the same POMs, no unsupported model expression occurs.
Expressions like `${project.guava.version}` there name properties, which
resolve today. The one model expression in a repository URL,
`${project.build.directory}`, sits in `distributionManagement`, which
...
- A POM that puts an unsupported model expression in a coordinate or a
  repository URL fails the resolve with a download error that names the
  literal `${...}`. Such a report is the reason to add that field, with
  Maven's inheritance and default for it.
